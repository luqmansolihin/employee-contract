...
@section('content')
    <div class="space-y-6">
        ...
        <!-- TailAdmin KPI Analytic Cards -->
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <!-- Total Karyawan -->
            <a href="{{ route('employees.index') }}"
                class="p-6 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs hover:border-[#3C50E0] hover:shadow-md transition group {{ !request('status') ? 'ring-2 ring-[#3C50E0] border-transparent' : '' }}">
                <div class="flex items-center justify-between">
                    <div
                        class="w-12 h-12 rounded-2xl bg-indigo-50 text-[#3C50E0] flex items-center justify-center group-hover:scale-110 transition">
                    ...
                    <span
                        class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-indigo-50 text-[#3C50E0]">Semua</span>
                </div>
                <div class="mt-4">
                    <p class="text-3xl font-extrabold text-[#1C2434]">{{ number_format($totalCount) }}</p>
                    <p class="text-xs font-medium text-slate-500 mt-1">Total Karyawan Kontrak</p>
                </div>
            </a>

            <!-- Kontrak Aktif -->
            <a href="{{ route('employees.index', array_merge(request()->query(), ['status' => 'active', 'page' => 1])) }}"
                class="p-6 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs hover:border-emerald-500 hover:shadow-md transition group {{ request('status') === 'active' ? 'ring-2 ring-emerald-500 border-transparent' : '' }}">
                <div class="flex items-center justify-between">
                    <div
                        class="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center group-hover:scale-110 transition">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <span
                        class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700">&gt;
                        30 Hari</span>
                </div>
                <div class="mt-4">
                    <p class="text-3xl font-extrabold text-emerald-900">{{ number_format($activeCount) }}</p>
                    <p class="text-xs font-medium text-slate-500 mt-1">Kontrak Aktif</p>
                </div>
            </a>

            <!-- Segera Habis -->
            <a href="{{ route('employees.index', array_merge(request()->query(), ['status' => 'expiring_soon', 'page' => 1])) }}"
                class="p-6 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs hover:border-amber-500 hover:shadow-md transition group {{ request('status') === 'expiring_soon' ? 'ring-2 ring-amber-500 border-transparent' : '' }}">
                <div class="flex items-center justify-between">
                    <div
                        class="w-12 h-12 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center group-hover:scale-110 transition">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <span
                        class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 flex items-center gap-1">
                        <span class="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span>
                        &le; 30 Hari
                    </span>
                </div>
                <div class="mt-4">
                    <p class="text-3xl font-extrabold text-amber-900">{{ number_format($expiringSoonCount) }}</p>
                    <p class="text-xs font-medium text-slate-500 mt-1">Segera Habis Kontrak</p>
                </div>
            </a>

            <!-- Habis Kontrak -->
            <a href="{{ route('employees.index', array_merge(request()->query(), ['status' => 'expired', 'page' => 1])) }}"
                class="p-6 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs hover:border-rose-500 hover:shadow-md transition group {{ request('status') === 'expired' ? 'ring-2 ring-rose-500 border-transparent' : '' }}">
                <div class="flex items-center justify-between">
                    <div
                        class="w-12 h-12 rounded-2xl bg-rose-50 text-rose-600 flex items-center justify-center group-hover:scale-110 transition">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <span
                        class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-rose-50 text-rose-700">Kadaluarsa</span>
                </div>
                <div class="mt-4">
                    <p class="text-3xl font-extrabold text-rose-900">{{ number_format($expiredCount) }}</p>
                    <p class="text-xs font-medium text-slate-500 mt-1">Kontrak Telah Berakhir</p>
                </div>
            </a>
        </div>
